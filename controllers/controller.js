...
const { password } = require('../Configuration/config');
async function startTimer(newTask,id){
    let sqlPool = await mssql.GetCreateIfNotExistPool(config)
    let request = new sql.Request(sqlPool)
    let newstartDate=new Date()
    newstartDate.setDate(newstartDate.getDate() - parseInt(newTask.duration.split(':')[0]))
    newstartDate.setHours(newstartDate.getHours() - parseInt(newTask.duration.split(':')[1]))
    newstartDate.setMinutes(newstartDate.getMinutes() - parseInt(newTask.duration.split(':')[2]))
    newstartDate.setSeconds(newstartDate.getSeconds() - parseInt(newTask.duration.split(':')[3]))
    //console.log(`UPDATE [dbo].[Task] SET [startDate]='${new Date(newstartDate).toISOString()}',[status]='true' WHERE id=${newTask.id}`);
    await request.query(`UPDATE [dbo].[Task] SET [startDate]='${new Date(newstartDate).toISOString()}',[status]='true' WHERE id=${newTask.id}`);
}
async function pauseTimer(oldTask,id){
    let sqlPool = await mssql.GetCreateIfNotExistPool(config)
    let request = new sql.Request(sqlPool)  
    var seconds = Math.floor(((new Date().getTime()) - (new Date(oldTask.startDate).getTime()))/1000);
...
module.exports.checkUser = async (req, res) => {
    ...
    let request = new sql.Request(sqlPool)
    try {
        //console.log(`select userName from User where userName='${req.body.userName}'`);
        let data=await request.query(`select userName from users where userName='${req.body.userName}'`);
...
module.exports.importTasks = async (req, res) => {
    ...
    let minMachineId=await (await request.query(`select min(id) from machine`)).recordset[0][""]
    console.log("req body " + req.body.MachinePath);
    await request.query(`
    ...
    if (await (await request.query(`select taskNumber from Machine where id=${minMachineId}`)).recordset[0].taskNumber==null || await (await request.query(`select taskNumber from Machine where id=${minMachineId}`)).recordset[0].taskNumber=='') 
        tasksOrder=taskId
    else

        tasksOrder=await (await request.query(`select taskNumber from Machine where id=${minMachineId}`)).recordset[0].taskNumber+","+taskId
    await request.query(`UPDATE [dbo].[Machine] SET [taskNumber]='${tasksOrder}' WHERE id=${minMachineId}`);
    res.json('inserted successfully')
}
module.exports.editOrder = async (req, res) => {
    ...
    if (await (await request.query(`select taskNumber from Machine where id=9`)).recordset[0].taskNumber==null || await (await request.query(`select taskNumber from Machine where id=9`)).recordset[0].taskNumber=='') 
        tasksOrder=taskId
    else

        tasksOrder=await (await request.query(`select taskNumber from Machine where id=9`)).recordset[0].taskNumber+","+taskId
    await request.query(`UPDATE [dbo].[Machine] SET [taskNumber]='${tasksOrder}' WHERE id=9`);
    res.json('inserted successfully')
}
