...
async function startTimer(newTask,id){
    ...
    console.log(`UPDATE [dbo].[Task] SET [endDate]='${new Date(newEndDate).toISOString()}',[status]='true' WHERE id=${newTask.id}`);
    await request.query(`UPDATE [dbo].[Task] SET [endDate]='${new Date(newEndDate).toISOString()}',[status]='true' WHERE id=${newTask.id}`);
}
